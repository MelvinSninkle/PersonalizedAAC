...
- **Tile framing** — every stored tile image is a standard square, baked at the image layer so all platforms render identically: uploads get a conservative uniform-border trim (only flat one-color bars, never content) + center square crop (skipped for keep-ratio posters), and the tile editor's **🖼 Adjust framing** overlay lets a parent drag/zoom the photo inside a tile-shaped frame and save exactly that square. Every reframe archives the prior picture — Previous pictures is the undo.
