...
- **Listening mode (live word-board)** — the parent taps **Listening mode** in the dashboard and the iPad's branding strip becomes a one-tile-high horizontal slider; anything said near the board is rendered there live as the child's own tiles, using the same greedy-longest matching as Message the Board (run client-side off the board's lexicon). Startable two ways: a **🎙️ Listen button on the board itself** (opposite the Play button, *not* edit-gated so the child can toggle it himself — turns red while listening), or remotely from the parent dashboard over `/api/live` (`listen-start`/`listen-stop`). Speech-to-text uses the native Capacitor `SpeechRecognition` plugin (Apple `SFSpeechRecognizer`), set to **on-device/offline** (`requiresOnDeviceRecognition`). Auto-stops after **2 minutes with no speech**. One-time native setup (incl. the offline enforcement one-liner) in `docs/listening-mode.md`.
- **Drag tiles across categories on the board** — in edit mode the iPad board already reordered tiles within a category by drag; now a tile can also be **dragged onto a category or subcategory tab to move it there**. Constrained to the same family — People stays with People, Nouns with Nouns, Verbs with Verbs (the three render as separate columns, so a tile can't jump sections). Re-parents via the same `serverUpdateItem` the parent organizer uses.
...
- **Access features, all three platforms** (admin dark-launch, parent dashboard → Access): **button navigation** replaces every scroll with whole-page paddle turns (eye-tracker / switch rigs; the tile that would be cut off becomes page N+1's first tile); the **sentence constructor** — MODAL, owned by a ✏️ pencil in the header: while on, the board runs button navigation (so nothing competes with scrolling) and a TAP stages its tile **silently** into the header bar — ▶ speaks the sentence, ✕ clears it, staging still logs so milestones see combinations; the mode turns itself off after 60s if no sentence gets started (parent-set idle clear 1–10 min for a bar left mid-sentence). Parents can additionally enable **drag staging** (`sentenceDrag`, Board → Board tools) on the iPad/Android apps: drag any tile up to the header to stage it — scrolling keeps working (native gesture systems run it alongside the scroll; the web keeps pencil-only because its version required disabling touch scroll). Header tools (🎙 listen / 📖 teach / 🙋 play / ✏️ pencil) are **parent-configurable per child** (Board → Board tools); **listening repeat-navigate** — a word heard twice in a row makes the board open that tile's category and flash it yellow.
...
- **One rule for every image add**: every photo surface (tile adds, tile editors, bulk category add, category icons, family portraits — web, iOS, Android) asks the same two-way question — **keep my exact photo** (free) or **draw it in the board's saved art style** (the default) — with no per-image style or model picking anywhere; changing the style is a deliberate act in the dashboard's Art style panel, and the server honors style/model overrides for admins only.
- **Live-ops: the New Style wizard** — one screen stands up a whole new offered style: upload one style image → approve a **generated demo kid** + objects reference (re-roll/upload) → 🚀 one button queues every default tile + folder icon to a **durable `style_build_jobs` queue** the every-minute cron drains (close the tab, it finishes) → review → **Publish**. Drafts never reach parents (invariant E9), and the **public practice board grows a style switcher** — every published style is instantly demo-able with zero extra renders. Plus: add-on boards are free-to-add everywhere (credits tier retired; web store gains the free-add rail), and a product-wide credits rule — every spend confirms "uses ⭐N, you have ⭐M" first, with balance badges on both native parent homes and ⭐-to-finish badges on every store category.
- **Listening profanity filter** (default ON, all three platforms): curse words and slurs said near a listening board render as the pill **"Bad Word"** instead of the word itself — the blocklist is server-owned (`api/_lib/bad-words.js`, shipped via `/api/sync` like match terms, so additions reach installed apps with no release). A second toggle hides **every** word that has no tile. Both are parent settings, editable in the dashboard, the web board's Display modal, and — a first — the native apps' parent Settings screens.
...
- **Tile framing** — every stored tile image is a standard square, baked at the image layer so all platforms render identically: uploads get a plain CENTER square crop (no automatic blank-space/border trimming of any kind — auto-trim was the board's most-complained-about behavior; skipped for keep-ratio posters), and the tile editors (web modal AND the native iOS sheet) have a **🖼 Adjust framing** overlay — drag/zoom the photo inside a tile-shaped frame and save exactly that square. Every reframe archives the prior picture — Previous pictures is the undo.
...
- **Public practice board** (`/practice`) — the starter board as a zero-auth, zero-spend demo with the REAL board's layout: pre-rendered demo audio (Lab `demo-audio` builder), voice chips, a style switcher over every published style, a "Meet:" switcher over each style's extra demo kids (`style_demo_children`; person tiles re-render per kid, families always get the primary set — demo_child_id 0 pinned in sync), localStorage-only stats, a session-only ⚙ Display modal (labels/sizes/sections/colors in sessionStorage — gone when the browser closes; opens pictures-first on a white frame), a timeboxed live listening demo (browser speech recognition, bad-word masking always on, nothing recorded), the Lab's curated default layout applied live, media whitelisted to the four shared prefixes; its entire server surface is two GETs. Landing page links it ("Try the board live").
...
- **Sentence constructor (documented per requirement #13)** — the child builds a sentence by moving tiles into the top bar, then ▶ speaks it (and logs it to the dashboard's Sentence activity panel). Two interaction modes: **Tap to add** (the accessibility-friendly default everywhere — with the ✏️ pencil on, a tap stages the tile; motor-limited children never need to drag) and **Pick up and drag** (the natural gesture — native iOS/Android only, behind the parent-enabled `sentenceDrag` setting; the web board is deliberately tap-only after drag-grabbing killed scrolling). Parent-tunable per child in synced settings; the repeat-say navigation threshold (#12: Off/2/3, `listenRepeatCount`) lives beside it in the Display panel. Known follow-up: re-tune the native drag pickup to the "short natural press" balance (a light touch must still scroll).
- **Reactive vocabulary + listening suggestions (#10)** — the taxonomy grows past the default board via store-only batches (`data/taxonomy-additions-2026-07.csv`, merged with the dedup-aware `taxonomy?fn=import-csv` — exact-label matches donate listen variants instead of duplicating). While listening runs, a silent second pass matches heard speech against the FULL canonical library (`/api/items?lexicon=1`): words the household says that the board lacks land in `word_suggestions` (one row per child+slug, hit-counted), reviewed on the parent dashboard's "Words your family says" panel — Add (Word-Shop path, ⭐1), Dismiss (resurfaces on next hearing), or Never (per-child suppression). Strictly consent-gated: `child_settings.suggestFromListening` is off until the parent's one-time opt-in (offered only in edit mode), toggleable in Display with a clear-pending offer, and re-checked server-side on every record — never audio, never transcripts, disclosed in privacy.html.
- **Movie & show tiles (#11, interim unlicensed)** — a find→link→parent-obtains-the-poster flow with a hard rule: **no studio artwork is ever fetched, cached, or served by app servers**; the only stored image is the parent's own upload. Find: `GET /api/items?movieSearch=` queries Wikidata (free, CC0) through `api/_lib/movie-search.js` — the single interface a licensed TMDB fetch replaces later with zero UX change; the parent always disambiguates from a title/year/type list. Link: `items.wikidata_qid` + `items.imdb_id` (also carried on `tile_jobs`); parent surfaces show a plain "Open in IMDb" URL, never the child board. Get the image: iOS `MovieAddSheet` opens the IMDb page in an in-app browser (long-press the poster → Add to Photos), with Photos-pick and camera (DVD case, shelf toy) fallbacks so the flow never dead-ends; the web edit modal links titles the same way. Render: poster tiles ride the normal durable pipeline use-as-is (raw, no AI redraw), keep their tall aspect, auto-file into the **TV & Movies** folder via the find-or-create folder hint, and speak the title in the child's own voice — so they work in every downstream feature like any other tile.
- **iPad demo board (#14)** — signing into the app with the literal user ID `admin` + the Vercel-set `ADMIN_TOKEN` (timing-safe server check; the token never ships in the client and **no session cookie is minted**) opens `DemoBoardView`: the same public starter-board projection the practice page uses, with live **style / demo-kid / demo-voice switchers** in the header — hand the iPad to a therapist and swap art styles mid-demo. Reads only public demo/media endpoints; demo mode is not persisted across relaunches.
- **Low-vision enlargement (#15)** — Display panel → "Bigger sizes": the listening-strip tiles and the big round header buttons each scale Normal / +50% / +100% independently, saved per device (`aacDisplay`); implemented as CSS scale variables so defaults are pixel-identical for everyone else.
- **Teaching facts on any tile (#16)** — the edit modal's three clue fields write `items.descriptive_clues`; a tile's own clues win over the canonical taxonomy overlay at sync, so a photo of Grandpa carries the family's real facts into Teach me and the games.
- **Quick-unlock PIN (#17, web)** — an optional per-device 4-digit PIN for the board's edit gate (SHA-256 device-salted in localStorage, auto-submit on the 4th digit, "Use password instead" always available, 5 failed tries fall back to the account password). Managed in Display → Safety; setting, changing, or removing the PIN re-verifies the account password server-side.
- **Copy cleanups** — `docs/emoji-audit.md` inventories every emoji in the codebase (review doc, nothing removed); customer-visible copy across web, iOS, Android, and API emails/errors is em-dash-free (rewritten to periods/commas/colons; code comments and en-dash ranges untouched).
