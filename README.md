...
- **Listening mode (live word-board)** — the parent taps **Listening mode** in the dashboard and the iPad's branding strip becomes a one-tile-high horizontal slider; anything said near the board is rendered there live as the child's own tiles, using the same greedy-longest matching as Message the Board (run client-side off the board's lexicon). Triggered over `/api/live` (`listen-start`/`listen-stop`); speech-to-text uses the native Capacitor `SpeechRecognition` plugin (Apple `SFSpeechRecognizer` — online now, on-device offline as a follow-up). One-time native setup in `docs/listening-mode.md`.
