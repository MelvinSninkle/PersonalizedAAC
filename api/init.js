...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Default (non-personalized) tile art. Any "default-able" tile — one that
    // never references a specific person ({reference}/{parent_photo}/{family_*},
    // not People/child_as_subject) — looks the same for every kid, so rather than
    // burn a per-child image generation on it we reuse ONE canonical image. The
    // admin one-time job (api/admin/lab?action=seed-defaults) copies the reference
    // board's (Fletcher's) already-drawn tile into a stable taxonomy-defaults/
    // Blob and records its key here. Onboarding's seed-core then points every
    // child's item at this shared key (still voicing the tile in the child's own
